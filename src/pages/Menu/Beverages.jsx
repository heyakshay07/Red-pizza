  ...
  return (
    <>
      <div className="max-w-6xl mx-auto px-4 py-10">
        <div className="text-center mb-10 mt-16">
          <h1 className="text-3xl font-extrabold text-gray-800">Beverages</h1>
          <p className="text-gray-600 mt-2">Cool and refreshing drinks to energize your day!</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {beverageItems.map((item) => (
            <div
              ...
              className="bg-white rounded-lg shadow-md hover:shadow-lg transition p-5 flex flex-col justify-between"
            >
              <img src={item.img} alt={item.name} className="w-full h-48 object-contain mb-4 rounded" />
              <h2 className="font-bold text-lg text-gray-800 mb-1">{item.name}</h2>
              <p className="text-gray-500 text-sm mb-3">{item.desc}</p>

              <div className="flex gap-4 mb-3">
                {item.sizes.includes("S") && <span>S - ₹{item.priceSmall}</span>}
                {item.sizes.includes("M") && item.priceMedium && <span>M - ₹{item.priceMedium}</span>}
              </div>
      ...
      {selectedItem && (
        <div className="fixed inset-0 flex justify-center items-center z-50" onClick={() => setSelectedItem(null)}>
          ...
          <div className="relative bg-white w-full max-w-sm rounded-lg shadow-lg p-6 z-10" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setSelectedItem(null)} className="absolute top-3 right-4 text-2xl text-gray-600 hover:text-black">
              ×
            </button>

            <img src={selectedItem.img} alt={selectedItem.name} className="w-full h-48 object-contain rounded mb-3" />
            <h2 className="text-xl font-bold mb-2">{selectedItem.name}</h2>
            <p className="text-gray-500 mb-4">{selectedItem.desc}</p>
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="text-right">
                <p className="text-gray-500 text-sm">Total</p>
                <p className="text-xl font-bold text-red-600">₹{(size === "M" ? selectedItem.priceMedium || selectedItem.priceSmall : selectedItem.priceSmall) * quantity}</p>
              </div>
            </div>
